...
const int mic1pin = A1; // Right
const int mic2pin = A6; // Left
const int mic3pin = A4; // Rear Centre
...
float energyThreshold = 1270000.00; // frame size 100 (6280000.00)  frame size 200 (12510000)
float crossingThreshold = 0.15;
...
////////////////////////////// Motor Constants //////////////////////////////
int baseSpeed = 127; // Base Speed that the motor will run at when not altered
int kp = 2; // Constant of Proportionality - (Adjust to change how aggressive motor angle change is)
...
void loop() {
  ...
  //Serial.print(count);
  // Remove outliers from angle_array[]
  ...
  // Compute the median of the filtered angles
  float median_angle = computeMedian(angle_array, count);

  //updateAngleBuffer(median_angle);
  float filtered_angle = computeMovingAverage();

  // For debugging: Print the median angle
  //Serial.print("Median Angle after outlier removal: ");
  //Serial.println(median_angle);
  //Serial.print("Moving Average Angle: ");
  //Serial.println(filtered_angle);


  //moveTowards(filtered_angle);

// compare angle collected to previous average/median. Theoretically shouldnt be a differance in the value of >10????
  motorControl();
}  
...
float angleErrorCorrection (float theta){
  // Coefficients for correction curve //
  float a = -3e-8;
  float b = 3e-7;
  float c = 6e-5;
  float d = -0.0012;
  ...
  //float error = a * pow(theta,5) + b * pow(theta,4) + c * pow(theta,3) + d * pow(theta,2) + e * theta + f;
  float error = theta *(theta *(theta *(theta *(theta *(a)+b)+c)+d)+e)+f;
...
bool isSoundPlayingSTE(float data[], int N, float energyThreshold) {
  int frameSize = 20;  // Number of samples per frame. Increase value to reduce sensitivity to peaks
...
void removeOutliers(float data[], int& size) {
  ...
  // Copy filtered data back into original array
...
void moveTowards(float angle){
  //float anglecons = constrain(angle,-90, 90);  // Ensure angle doesnt exceed maximum 
  int adjustment = angle * kp; 
  updateMotors(adjustment);
  //turnTime = // function to calc how long to turn for based on angle
  int turnTime = 500;
  delay(turnTime); // Delay value to change and become variable based on a look up table!!!!!!!!!!
...
void pinSet(int pin, bool pinOn){
  if (pinOn == true){
